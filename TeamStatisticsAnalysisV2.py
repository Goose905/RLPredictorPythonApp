#Author: Luke Zurad
#Last Modified: 12/04/2025 - Updated to reflect underscores in headers instead of spaces
#Purspose:
...
numeric_cols = [c for c in df.columns if any(
    c.startswith(f"P{i}_") and 
    not c.endswith("_player_name") and 
    not c.endswith("_car_name") and 
    "_car_id" not in c
    for i in range(1, 7)
)]
# Split into winning/losing team columns
win_cols  = [c for c in numeric_cols if c.startswith(("P4_", "P5_", "P6_"))]
lose_cols = [c for c in numeric_cols if c.startswith(("P1_", "P2_", "P3_"))]
...
# Remove any column containing 'car_id'
team_df = team_df[[c for c in team_df.columns if "car_id" not in c.lower()]]
